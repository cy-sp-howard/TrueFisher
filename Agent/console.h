...
public:
	...
	static Console_PARAMETERS GetDefaultParameters();


public:
	Console(HINSTANCE hModule = NULL);
	~Console();

	// opens console and starts handling of all events
	// waits for private thread to run (eg. will deadlock when called in DllMain)
	bool create(const std::string& windowTitle, Console_PARAMETERS* parameters = nullptr);
...
private:
	...
	std::string m_title;
	Console_PARAMETERS m_parameters = GetDefaultParameters();

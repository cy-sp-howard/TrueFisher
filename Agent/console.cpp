...
static const char WNDCLASS_Console[] = "Console";
...
Console_PARAMETERS Console::GetDefaultParameters()
{
...
Console::Console(HINSTANCE hModule)
{
	if (hModule)
		m_hModule = hModule;
	else
	...
	if (!g_wndClassRefCount++)
	{
		...
		wc.lpszClassName = WNDCLASS_Console;
...
Console::~Console()
{
	...
	if (!--g_wndClassRefCount)
		UnregisterClassA(WNDCLASS_Console, m_hModule);
}


bool Console::create(const std::string& windowTitle, Console_PARAMETERS* parameters)
{
	...
	m_thread = std::thread(&Console::threadProc, this, std::ref(p));
...
void Console::registerInputCallback(void (*cbInput)(std::string))
{
	m_cbInput = cbInput;
}
...
HWND Console::getWindowHandle() const
{
	return m_hWnd;
}


void Console::vprintf(const char* format, va_list valist)
{
	va_list valist_copy;
...
// WndProc for Console window class
LRESULT CALLBACK Console::s_WndProc(HWND hWnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	Console* pInstance = reinterpret_cast<Console*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
...
LRESULT Console::wndProc(HWND hWnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch (msg)
	{
	case WM_CLOSE:
...
void Console::threadProc(std::promise<bool>& p)
{
	...
	m_hWnd = CreateWindowA(WNDCLASS_Console, m_title.c_str(),
...
void Console::writeStringToRawBuffer(char* strOut)
{
...
void Console::ConsoleBuffer::free()
{
	if (m_bufferHandle)
	{
...
HLOCAL Console::ConsoleBuffer::getBuffer() const
{
	return m_bufferHandle;
}
void Console::ConsoleBuffer::setBuffer(HLOCAL buffer)
{
	if (!m_bufferHandle)
	{
...
void Console::ConsoleBuffer::lock()
{
	m_writeMutex.lock();
	m_buffer = reinterpret_cast<wchar_t*>(LocalLock(m_bufferHandle));
}
void Console::ConsoleBuffer::unlock()
{
...
bool Console::ConsoleBuffer::resize(int width, int height)
{
	if (!m_bufferHandle)
	{
		return false;
...
void Console::ConsoleBuffer::clear()
{
	if (m_buffer)
	{
...
void Console::ConsoleBuffer::write(wchar_t* str)
{
	if (m_buffer)
	{
...
void Console::ConsoleBuffer::scrollUp()
{
	if (m_buffer)
	{
...
int Console::ConsoleBuffer::getBufferOffsetFromCursorPos()
{

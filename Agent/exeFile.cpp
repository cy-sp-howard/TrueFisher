...
std::vector<uintptr_t> LoadRelocs(uintptr_t relocData)
{
...
bool ExeFile::loadFromMem(uintptr_t moduleBase)
{
	m_valid = false;
...
bool ExeFile::loadFromFile(const std::string& path)
{
...
bool ExeFile::isReloc(uintptr_t rva) const
{
	if (!m_valid)
	{
		throw std::runtime_error("no exe file loaded");
...
uintptr_t ExeFile::getExport(const std::string& name) const
{
